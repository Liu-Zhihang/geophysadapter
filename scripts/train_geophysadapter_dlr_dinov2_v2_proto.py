#!/usr/bin/env python3
"""GeoPhysAdapter v2 on DLR strict_t3.

Changes over v1:
- high-resolution detail branch on raw pre/post/change stacks
- physics-conditioned FiLM modulation on semantic/detail features
- cross-attention refinement at the fused DINO semantic map
- optional mixed precision for faster smoke validation on 16 GB GPUs
"""
...
class GeoPhysAdapterDecoderV2(nn.Module):
    def __init__(self, embed_dim: int, token_dim: int = 128, sem_ch: int = 96, detail_ch: int = 64, terrain_ch: int = 64):
        super().__init__()
        ...
        self.seg_head = nn.Conv2d(128, 1, 1)
    ...
    def forward(
        ...
        fused = self.final_fuse(torch.cat([sem, detail, terr], dim=1))
        logits = self.seg_head(fused)
        states = self.state_head(fused)
        return logits, states[:, :1], states[:, 1:2]


class GeoPhysAdapterHybridPINNV2(nn.Module):
    def __init__(self, dino_model: str, pretrained_backbone: bool, freeze_backbone: bool, unfreeze_last_blocks: int, dino_input_size: int):
        super().__init__()
        ...
        self.decoder = GeoPhysAdapterDecoderV2(embed_dim=self.visual.embed_dim, token_dim=128, sem_ch=96, detail_ch=64, terrain_ch=64)
...
def run_epoch(
    ...
    for batch in loader:
        ...
        with torch.autocast(device_type=device.type, dtype=autocast_dtype, enabled=autocast_enabled):
            logits, u_hat, fos_hat = model(pre, post, terrain, material, trigger)
...
def main() -> int:
    ...
    model = GeoPhysAdapterHybridPINNV2(
    ...
    for epoch in range(1, args.epochs + 1):
        t0 = time.time()
        tr = run_epoch(model, train_loader, device, optimizer, scaler, args.amp, args.lambda_change, args.lambda_topo, args.lambda_hydro, args.lambda_stability, args.lambda_obs, slope_low_thresh_norm, slope_temp_norm, args.ndvi_drop_thresh, args.ndvi_temp)
        va = run_epoch(model, val_loader, device, None, None, args.amp, args.lambda_change, args.lambda_topo, args.lambda_hydro, args.lambda_stability, args.lambda_obs, slope_low_thresh_norm, slope_temp_norm, args.ndvi_drop_thresh, args.ndvi_temp)
        scheduler.step()
        sec = time.time() - t0
    ...
    metrics = {
        "val": run_epoch(model, val_loader, device, None, None, args.amp, args.lambda_change, args.lambda_topo, args.lambda_hydro, args.lambda_stability, args.lambda_obs, slope_low_thresh_norm, slope_temp_norm, args.ndvi_drop_thresh, args.ndvi_temp),
        "testind": run_epoch(model, testind_loader, device, None, None, args.amp, args.lambda_change, args.lambda_topo, args.lambda_hydro, args.lambda_stability, args.lambda_obs, slope_low_thresh_norm, slope_temp_norm, args.ndvi_drop_thresh, args.ndvi_temp),
        "testspt": run_epoch(model, testspt_loader, device, None, None, args.amp, args.lambda_change, args.lambda_topo, args.lambda_hydro, args.lambda_stability, args.lambda_obs, slope_low_thresh_norm, slope_temp_norm, args.ndvi_drop_thresh, args.ndvi_temp),
    }
    result = {
        "timestamp": int(time.time()),
        "seed": args.seed,
        "device": str(device),
        "architecture": "GeoPhysAdapterHybridPINNV2",
        "dino_model": args.dino_model,
        "dino_input_size": args.dino_input_size,
        ...
        "lambdas": {"change": args.lambda_change, "topo": args.lambda_topo, "hydro": args.lambda_hydro, "stability": args.lambda_stability, "obs": args.lambda_obs},
