...
cd "$ROOT"
"$PYTHON" physics_informed_landslide_dataset/scripts/xdomain/audit_pild_xdomain_local_assets.py

echo "[$(date --iso-8601=seconds)] extracting and checksum-verifying S2 archives"
"$PYTHON" physics_informed_landslide_dataset/scripts/xdomain/extract_sen12_s2.py --workers 8

echo "[$(date --iso-8601=seconds)] indexing S2 samples and physical-event groups"
"$PYTHON" physics_informed_landslide_dataset/scripts/xdomain/index_sen12_s2.py --workers 24

echo "[$(date --iso-8601=seconds)] refreshing cross-source event registry"
"$PYTHON" physics_informed_landslide_dataset/scripts/xdomain/build_pild_xdomain_event_registry.py

echo "[$(date --iso-8601=seconds)] building the audited 4,979-sample change-view S2/Terrain cache"
"$PYTHON" physics_informed_landslide_dataset/scripts/xdomain/build_sen12_s2_tmr_cache.py

echo "[$(date --iso-8601=seconds)] running the real-data CUDA visual/adapter smoke"
OUTBASE="$ROOT/physics_informed_landslide_dataset/experiments/revision2026/sen12_xdomain_geophysadapter_smoke_v1" \
FOLDS="0" SEEDS="20260721" GPUS="0" EPOCHS="2" MAX_STEPS="20" \
NUM_WORKERS="2" \
bash physics_informed_landslide_dataset/scripts/xdomain/run_sen12_xdomain_geophysadapter.sh

echo "[$(date --iso-8601=seconds)] launching the formal five-fold five-seed dual-GPU matrix"
OUTBASE="$ROOT/physics_informed_landslide_dataset/experiments/revision2026/sen12_xdomain_geophysadapter_v1" \
...
"$PYTHON" physics_informed_landslide_dataset/scripts/xdomain/analyze_sen12_xdomain_geophysadapter.py \
  --runs-dir "$ROOT/physics_informed_landslide_dataset/experiments/revision2026/sen12_xdomain_geophysadapter_v1"
