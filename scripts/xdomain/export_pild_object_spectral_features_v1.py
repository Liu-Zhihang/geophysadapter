...
def normalized_difference (a :np .ndarray ,b :np .ndarray )->np .ndarray :
    """Normalized difference index with EPS in the denominator."""
    return (a -b )/(a +b +EPS )


def spectral_index_stack (cube :np .ndarray )->dict [str ,np .ndarray ]:
    """Compute interpretive indices from a 6-band cube of shape (6, H, W)."""
    ...
    # Bare-soil index: rises when scar material is exposed
...
ring_radius :int ,
    ...
    """Spectral descriptors for one connected component, including a ring contrast."""
    row_slice ,col_slice =window 
    # Pad for the surrounding ring
