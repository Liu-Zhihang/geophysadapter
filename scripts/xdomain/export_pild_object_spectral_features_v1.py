...
def process_fold(
    ...
    每折的掩膜阈值不同（0.92 / 0.95 / 0.805 / 0.92），由该折的 receipt 决定，
    必须逐折读取才能与诊断分量表逐体对齐。
    """
    ...
    if optical_ids != sample_id:
        raise RuntimeError(f"{fold_id}: 光学缓存与预测缓存的样本顺序不一致")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="留空则逐折从 receipt 读取（与诊断分量表一致）",
    )
    ...
    for fold_id in FOLD_IDS:
        ...
        print(f"{fold_id}: 阈值 {thresholds[fold_id]:.3f}，{len(rows):,} 个对象")
    ...
    print(f"\n共 {len(table):,} 个对象，{len(feature_cols)} 个光谱描述子")
