...
# ROLE_MATERIAL_FEATURE_NAMES order: five AWC depths, then texture stacks
AWC_SLICE = slice(0, 5)
CLAY_IDX = (5, 6)
SAND_IDX = (7, 8)
CFVO_IDX = (17, 18)
...
def robust_unit(values: np.ndarray, mask: np.ndarray) -> np.ndarray:
    """Robust normalize by in-mask 95th percentile; clip to [0, 1]."""
    if not mask.any():
...
def rank01(values: np.ndarray) -> np.ndarray:
    """Rank-normalize to [0, 1] from features only; no labels."""
...
def boundary_strength(
    ...
    """Sample boundary strength: divide + aspect break + slope break."""
