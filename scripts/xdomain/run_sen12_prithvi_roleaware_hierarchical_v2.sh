...
OUTROOT="${OUTROOT:-$PROJECT/experiments/revision2026/sen12_prithvi_roleaware_hierarchical_v2}"
...
PRESERVATION_WEIGHT="${PRESERVATION_WEIGHT:-0.10}"
PRESERVATION_CONFIDENCE="${PRESERVATION_CONFIDENCE:-0.90}"
MIN_SELECTION_SCORE_GAIN="${MIN_SELECTION_SCORE_GAIN:-0.001}"
MIN_SELECTION_NET_ERROR_REDUCTION="${MIN_SELECTION_NET_ERROR_REDUCTION:-1}"
DRY_RUN="${DRY_RUN:-0}"
...
make_command() {
  ...
  COMMAND=(
    "$PYTHON" "$SCRIPT_DIR/train_sen12_prithvi_roleaware_hierarchical_v2.py"
  ...
  if [[ "$VALIDATE_ONLY" == "1" ]]; then
    ...
    COMMAND+=(
      ...
      --preservation-weight "$PRESERVATION_WEIGHT"
      --preservation-confidence "$PRESERVATION_CONFIDENCE"
      --min-selection-score-gain "$MIN_SELECTION_SCORE_GAIN"
      --min-selection-net-error-reduction "$MIN_SELECTION_NET_ERROR_REDUCTION"
    )
  fi
}
