...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="该视觉锚点自身的整池基线 IoU；换锚点时必须随之更换",
    )
    parser.add_argument(
    ...
    if len(frame) != len(whole):
        raise RuntimeError(f"对齐失败：{len(whole)} -> {len(frame)}")
    ...
    print(
        f"对象 {len(frame):,}  事件 {frame.canonical_event_id.nunique()}  "
        f"特征 {len(feature_cols)}（地形 {len(TERRAIN)} / 置信 {len(CONFIDENCE)} / "
        f"光谱 {len(spec_cols)} / 水文 {len(hyd_cols)}）"
    )
    print(f"整池 TP={tp:,.0f} FP={fp:,.0f} FN={fn:,.0f}  baseline IoU={base_iou:.5f}  cut={cut:.5f}\n")
    ...
    for name, x in (("source_conditioned", x_source), ("source_blind", x_plain)):
        ...
        print(
            ...
            f"移除精度={outcome['removal_precision']:.3f}\n"
            f"  FP 质量清除 {outcome['fp_mass_captured']:.1%}，TP 质量损失 {outcome['tp_mass_lost']:.1%}\n"
            f"  事件宏观 ΔIoU={outcome['event_macro_delta_iou']:+.5f} [{d_lo:+.5f}, {d_hi:+.5f}]  "
            f"RER={outcome['event_macro_rer']:+.4f} [{r_lo:+.4f}, {r_hi:+.4f}]  "
            f"{outcome['events_positive']}/{outcome['n_events']} 事件为正\n"
            f"  源宏观 ΔIoU={outcome['source_macro_delta_iou']:+.5f}"
        )
        print("  逐源：")
