...
def main() -> None:
    ...
    print(
        ...
        f"features={len(feature_cols)} (terrain={len(TERRAIN)} / confidence={len(CONFIDENCE)} / "
    ...
    for name, x in (("source_conditioned", x_source), ("source_blind", x_plain)):
        ...
        print(
            ...
            f"removal_precision={outcome['removal_precision']:.3f}\n"
            f"  FP mass cleared {outcome['fp_mass_captured']:.1%}, TP mass lost {outcome['tp_mass_lost']:.1%}\n"
            f"  event-macro ΔIoU={outcome['event_macro_delta_iou']:+.5f} [{d_lo:+.5f}, {d_hi:+.5f}]  "
            f"RER={outcome['event_macro_rer']:+.4f} [{r_lo:+.4f}, {r_hi:+.4f}]  "
            f"{outcome['events_positive']}/{outcome['n_events']} events positive\n"
            f"  source-macro ΔIoU={outcome['source_macro_delta_iou']:+.5f}"
        )
        print("  by source:")
